...
public static class WorkoutEndpoints
{
    public static void MapWorkoutEndpoints(this WebApplication app)
    {
        ...
        group
            .MapPost(
                "/",
                async (Guid profileId, CreateWorkoutRequest request, FitTrackerDbContext db) =>
                {
                    ...
                    var workout = new Workout
                    {
                        ...
                        Days = request.Days.ConvertAll(ParseDayOfWeek),
                        UserProfileId = profileId,
                        CreatedAt = DateTime.UtcNow,
                        UpdatedAt = DateTime.UtcNow,
                        Exercises = request.Exercises.ConvertAll(e => new Exercise
                        {
                            Id = Guid.NewGuid(),
                            Name = e.Name,
                    ...
                    return Results.Created(
                        $"/api/profiles/{profileId}/workouts/{workout.Id}",
                        ToDto(workout)
                    );
                }
            )
        ...
        group
            .MapPut(
                ...
                ) =>
                {
                    ...
                    var workout = await db.Workouts.FirstOrDefaultAsync(w =>
                        w.Id == id && w.UserProfileId == profileId
                    );
                    ...
                    workout.Days = request.Days.ConvertAll(ParseDayOfWeek);
                    ...
                    await db.Exercises.Where(e => e.WorkoutId == id).ExecuteDeleteAsync();

                    // Criar novos exercícios
                    var newExercises = request.Exercises.ConvertAll(e => new Exercise
                    {
                        Id = Guid.NewGuid(),
                        Name = e.Name,
                        MuscleGroup = ParseMuscleGroup(e.MuscleGroup),
    ...
    private static WorkoutDto ToDto(Workout workout) =>
        new(
            ...
            workout.Days.ConvertAll(d => d.ToString().ToLower()),
            workout.Exercises.ConvertAll(ToExerciseDto),
            workout.CreatedAt,
            workout.UpdatedAt,
            workout.IsCompleted,
            workout.CompletedAt
    ...
    private static WorkoutGoal ParseWorkoutGoal(string goal) =>
        goal.ToLower() switch
        {
    ...
    private static DayOfWeekEnum ParseDayOfWeek(string day) =>
        day.ToLower() switch
        {
    ...
    private static DayOfWeekEnum MapSystemDayToEnum(DayOfWeek day) =>
        day switch
        {
