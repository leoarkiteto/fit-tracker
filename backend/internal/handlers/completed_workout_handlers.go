...
func (h *CompletedWorkoutHandler) GetStats(w http.ResponseWriter, r *http.Request) {
	...
	err := h.DB.QueryRow(`SELECT COUNT(*) FROM CompletedWorkouts WHERE user_profile_id = ?`, profileID).
		Scan(&totalWorkouts)
	if err != nil {
		serverError(w, err)
		return
	...
	err = h.DB.QueryRow(`SELECT COUNT(*) FROM CompletedWorkouts WHERE user_profile_id = ? AND completed_at >= ?`, profileID, lastWeek).
		Scan(&workoutsThisWeek)
	if err != nil {
		serverError(w, err)
		return
	}
	...
	err = h.DB.QueryRow(`SELECT COALESCE(SUM(duration_seconds), 0) FROM CompletedWorkouts WHERE user_profile_id = ?`, profileID).
		Scan(&totalSeconds)
	if err != nil {
		serverError(w, err)
		return
