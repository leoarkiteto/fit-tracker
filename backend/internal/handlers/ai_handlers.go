...
func (h *AIHandler) Generate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(w, "Ollama is not reachable or model not loaded. Make sure Ollama is running (ollama serve) and the model is pulled (ollama pull llama3.2).", http.StatusServiceUnavailable)
		return
	}

	// Wrap the raw AI output with server-generated fields (planId, generatedAt)
	var aiOutput map[string]interface{}
	if err := json.Unmarshal([]byte(responseText), &aiOutput); err != nil {
		log.Printf("AI returned invalid JSON: %s, error: %v", responseText, err)
		http.Error(w, "AI returned invalid JSON. The model may still be loading. Please try again.", http.StatusInternalServerError)
		return
	}
...
func (h *AIHandler) Status(w http.ResponseWriter, r *http.Request) {
	// Verify Ollama is actually reachable with a lightweight call
	ctx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
	defer cancel()

	err := h.AIClient.CheckHealth(ctx)
	available := err == nil

	provider := "Ollama (via LangChainGo)"
	model := h.AIClient.Model
	endpoint := h.AIClient.Endpoint
	if !available {
