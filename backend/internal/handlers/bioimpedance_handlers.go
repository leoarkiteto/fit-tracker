...
func (h *BioimpedanceHandler) GetAll(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query(
		`SELECT id, date, weight, body_fat_percentage, muscle_mass, bone_mass, water_percentage, visceral_fat, bmr, metabolic_age, notes FROM BioimpedanceData WHERE user_profile_id = ? ORDER BY date DESC`,
		profileID,
	)
	if err != nil {
		serverError(w, err)
