...
func (h *ProfileHandler) GetAll(w http.ResponseWriter, r *http.Request) {
	rows, err := h.DB.Query(
		`SELECT id, name, age, height, current_weight, goal_weight, avatar_url, experience_level, available_days_per_week, preferred_workout_duration, equipment_type FROM UserProfiles`,
	)
	if err != nil {
		serverError(w, err)
		return
...
func (h *ProfileHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.DB.Exec(
		...
		req.AvatarURL,
		req.ExperienceLevel,
		req.AvailableDaysPerWeek,
		req.PreferredWorkoutDuration,
		req.EquipmentType,
		time.Now(),
		id,
	)
