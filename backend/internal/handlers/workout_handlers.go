...
func (h *WorkoutHandler) GetAll(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query(
		`SELECT id, name, description, goal, days, created_at, updated_at, is_completed, completed_at FROM Workouts WHERE user_profile_id = ? ORDER BY created_at DESC`,
		profileID,
	)
	if err != nil {
		serverError(w, err)
...
func (h *WorkoutHandler) GetToday(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query(
		`SELECT id, name, description, goal, days, created_at, updated_at, is_completed, completed_at FROM Workouts WHERE user_profile_id = ? AND ',' || LOWER(days) || ',' LIKE '%,' || ? || ',%'`,
		profileID,
		today,
	)
	if err != nil {
...
func (h *WorkoutHandler) loadExercises(workoutID string) ([]models.Exercise, error) {
	rows, err := h.DB.Query(
		`SELECT id, name, muscle_group, sets, reps, weight, rest_seconds, notes FROM Exercises WHERE workout_id = ?`,
		workoutID,
	)
	if err != nil {
		return nil, err
...
func (h *WorkoutHandler) loadExercisesBatch(
	workoutIDs []string,
) (map[string][]models.Exercise, error) {
