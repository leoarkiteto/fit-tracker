...
// testProfileID is the profile every hydration test scopes to.
const testProfileID = "profile-1"

// newTestService builds the hydration service on an in-memory repository.
func newTestService(t *testing.T, repo water.Repository) *Service {
	t.Helper()
	return New(repo)
}
