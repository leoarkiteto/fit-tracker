...
func TestGetAll_EchoesProfileID(t *testing.T) {
	...
	repo.History = []bioimpedance.Measurement{
		{ID: "measurement-1", Weight: 78.2, BodyFatPercentage: 18.4},
	}
	...
	if history[0].BodyFatPercentage != 18.4 {
		t.Errorf("BodyFatPercentage = %v, want 18.4", history[0].BodyFatPercentage)
	}
}
